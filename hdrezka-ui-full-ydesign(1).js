...
(function () {
    ...
    function markProcessed(html) {
        if (html && html.setAttribute) html.setAttribute('data-hdy', '1');
    }

    function isProcessed(html) {
        return !!(html && html.getAttribute && html.getAttribute('data-hdy') === '1');
    }

    function clearOldOverlay(view) {
        if (!view) return;
        var old = view.querySelectorAll('.hdy-overlay, .card__badge-hdrezka');
            ...
            if (isOn('hdy_show_logo') && ext && ext.logo) {
                ...
                logoImg.onerror = function () {
                    try {
                        ...
                        if (isOn('hdy_show_title') && title && !overlay.querySelector('.hdy-title')) {
                            var t = document.createElement('div');
                            t.className = 'hdy-title';
                            t.textContent = title;
                            overlay.insertBefore(t, overlay.firstChild);
                        }
                    } catch (e) {}
                };
